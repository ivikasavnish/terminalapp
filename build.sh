...
mv ./build/bin/${APP_NAME}.exe $OUTPUT_DIR/${APP_NAME}-${VERSION}-windows-amd64.exe
...
mv ./build/bin/${APP_NAME}.app $OUTPUT_DIR/${APP_NAME}-${VERSION}-macos.app
...
mv ./build/bin/${APP_NAME} $OUTPUT_DIR/${APP_NAME}-${VERSION}-linux-amd64
...
' > /app/deb/DEBIAN/control
  dpkg-deb --build /app/deb $OUTPUT_DIR/${APP_NAME}-${VERSION}-amd64.deb
"
...
' > /app/rpm/SPECS/${APP_NAME}.spec
  ...
  cp /app/rpm/RPMS/x86_64/${APP_NAME}-${VERSION}-1.x86_64.rpm $OUTPUT_DIR/
"
